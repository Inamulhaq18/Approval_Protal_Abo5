...
for index, row in pfa.iterrows():
  ...
  try:
      templist=[]
      if "imgsource" in row["variety"]:
         print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
  ...
  st.write(len(handler),len(Title),len(Body),len(vendor_),len(CPT),len(tags),len(option1),len(option1val),len(Published),len(price),len(imageposition),len(image_link),len(imagevurl))
