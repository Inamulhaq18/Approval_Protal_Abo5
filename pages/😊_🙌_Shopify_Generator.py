...
#   st.write(len(handler),len(Title),len(Body),len(vendor_),len(CPT),len(tags),len(option1),len(option1_val),len(Published),len(pr_ice),len(imageposition),len(image_link),len(imagevurl))
  alllen=len(handler)
  if len(handler)==len(Title)==len(Body)==len(vendor_)==len(CPT)==len(tags)==len(option1)==len(option1_val)==len(Published)==len(pr_ice)==len(imageposition)==len(image_link)==len(imagevurl):
